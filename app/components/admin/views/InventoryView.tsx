        ...
        <div className="space-y-6 relative">
            ...
            <div className={`p-4 rounded-3xl border ${base.card} flex flex-col gap-3`}>
                ...
                <div className="flex gap-2 flex-wrap">
                    <input type="text" value={newIngName} onChange={e => setNewIngName(e.target.value)} placeholder="Nombre..." className={`flex-[2] p-3 rounded-xl outline-none border min-w-[120px] ${base.input}`} />
                            ...
                            {isEditing ? (
                                <div className="p-4 space-y-3">
                                    ...
                                    <div className="flex gap-2 mt-3">
                                        <button onClick={cancelEditIng} className={`flex-1 ${base.buttonSec} border p-2 rounded-lg flex items-center justify-center`}><X size={16}/></button>
                                        <button onClick={() => saveEditIng(ing.id)} className={`flex-[2] ${currentTheme.color} text-white p-2 rounded-lg flex items-center justify-center shadow`}><Save size={16}/></button>
                                    </div>
                                </div>
                            ) : (
                                <>
                                    <div className="p-4 pb-2 relative flex-1">
                                        <div className="flex justify-between items-start mb-1">
                                            <div className="min-w-0 pr-12"> 
                                                <h3 className="font-bold text-sm overflow-x-auto whitespace-nowrap no-scrollbar opacity-90">{ing.nombre}</h3>
                                    ...
                                    <div className="px-2 pb-2 pt-0">
                                        <div className="flex gap-1 justify-between">
                                            {[1, 5, 10, 100].map((amt) => (
                                                <button key={amt} onClick={() => quickUpdateStock(ing.id, ing.cantidad_disponible, amt)} className={`flex-1 py-2 text-[10px] font-bold opacity-50 hover:opacity-100 rounded active:scale-95 transition-all ${incrementBtnClass}`}>+{amt}</button>
                                            ))}
                                        </div>
                                    </div>
                                </>
                ...
                <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in p-4 pb-24 sm:pb-4">
                    ...
                    <div className={`${base.card} w-full max-w-sm rounded-[32px] p-6 shadow-2xl relative border z-10 animate-in slide-in-from-bottom-10 duration-300`}>
                        ...
                        <div className="space-y-3">
                            <button onClick={handleWhatsAppShare} className="w-full py-4 rounded-2xl flex items-center justify-center gap-3 font-bold bg-[#25D366] text-white shadow-lg active:scale-95 transition-transform hover:brightness-110">
                                <MessageCircle size={24} /> Enviar por WhatsApp
                            </button>
                            <button onClick={handleNativeShare} className={`w-full py-4 rounded-2xl flex items-center justify-center gap-3 font-bold border ${base.buttonSec} active:scale-95 transition-transform`}>
